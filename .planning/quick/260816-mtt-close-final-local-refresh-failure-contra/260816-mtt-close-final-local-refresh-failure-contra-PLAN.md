...
must_haves:
  truths:
    ...
    - "Every failure after a successfully published claim—adapter construction, `claim_attachment`, `lockfile_plan_materialization`, local Docker authority, collector, both builds, migration, schema, cutover, routes, release, rollback, evidence, `failure_recollection` and failure-report publication—retains the canonical claim and yields a strict sanitized report or an artifact-specific unrecoverable invariant."
  ...
  key_links:
    ...
    - from: scripts/refresh-local.test.mjs
      ...
      pattern: "adapter_construction|claim_attachment|lockfile_plan_materialization|failure_recollection|rollback-api-web|write-evidence|UNRECOVERABLE"
  prohibitions:
...
- Preserve the empty-argv order: resolve clean full Git revision; assert claim absent; atomically publish claim; only after `claimRefreshAttempt` returns enter the failure-report `try`. Claim publication failures stay outside that `try` and cannot call `writeFailureReport`; adapter construction, attaching the returned claim, raw lockfile read/hash/plan materialization and every later local-authority/collector/build/mutation/recollection/evidence operation remain inside it. A returned claim is never removed.
- Use a table-driven terminal-stage harness over exact stages `adapter_construction`, `claim_attachment`, `lockfile_plan_materialization`, `local_docker_authority`, `preflight_collection`, `build-api`, `build-web`, `migrate`, `schema-verify`, `cutover-api-web`, `routes`, `release-blocked`, `rollback-api-web`, `verify-rollback`, `write-evidence`, `failure_recollection` and `failure_report_publication`. For every injected failure after claim return, assert canonical claim-present bytes/digest, no same-revision adapter retry, sanitized report bound to that digest when publication succeeds, evidence absent unless its own cleanup is explicitly unrecoverable, exact one-off cleanup where applicable, and correct proved/not-applicable/unproved preservation semantics. `claim_attachment` and `lockfile_plan_materialization` failures use pre-runtime preservation semantics; a `failure_recollection` error must still publish a bound sanitized report with `recollection: failed` and `preservation: unproved`, unless report publication itself raises `UNRECOVERABLE_FAILURE_REPORT_INVARIANT`. Report writer failure retains the claim and original failure as cause.
...
| Independent audit | Every post-claim stage, including claim attachment, lockfile/plan materialization and failed recollection, is terminal and truthfully reported | Covered | 1 RED stage table; 2 terminal boundary |
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - "Terminal cases: each exact named post-claim stage preserves the claim and publishes a bound sanitized report or explicit unrecoverable invariant; this explicitly includes `claim_attachment`, `lockfile_plan_materialization` and `failure_recollection`, while claim-publication failure writes no report."
    - "Recollection cases: when `recollectFailure` itself throws, the canonical claim remains and a digest-bound sanitized report records `recollection: failed` plus `preservation: unproved`; only report-publication failure may replace that outcome with the explicit unrecoverable invariant."
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - "All named post-claim stage failures, including `claim_attachment`, `lockfile_plan_materialization` and `failure_recollection`, are terminal for the revision and durably report safe preservation status or an explicit invariant."
  ...
  <action>
    Implement the smallest refactor satisfying the RED suite. Extract shared raw-boundary runtime assembly without exporting precomputed fact/probe seams; keep production wrappers zero-argument and make the test-only wrapper the only injectable test entry. Bind report-present verification to a freshly read canonical claim and exact digest. Keep claim publication lexically outside the report `try`, then route adapter construction, claim attachment, lockfile hash/plan materialization, local authority/collector/build/migration/schema/cutover/routes/release/rollback/evidence and failure recollection through one stage-aware terminal reporter. A recollection exception must be converted to the bound sanitized failed/unproved report before attempting atomic publication; publication failure alone escalates to the explicit unrecoverable invariant. Harden the common claim/report writer and evidence writer with artifact-specific cleanup/state invariants across the full operation matrix. Preserve current evidence v4, Docker argv/local-authority, same-origin route and persistence behavior without expanding scope. Commit the GREEN implementation and any necessary test refactor as `fix(quick-260816-mtt): close final refresh failure contracts`.
  </action>
  <verify>
  ...
  <done>All four audit gaps are GREEN in one atomic implementation commit; the exact stage matrix proves `claim_attachment`, `lockfile_plan_materialization` and `failure_recollection` preserve the canonical claim and produce the required bound report/invariant; existing behavior remains GREEN, protected/runtime artifacts are byte-identical and release remains BLOCKED.</done>
</task>
...
| T-Q-03 | Repudiation | post-claim terminal failures | high | mitigate | Exact table-driven coverage includes claim attachment, lockfile/plan materialization and failed recollection, with persistent claim and sanitized bound report or explicit invariant. |
...
- Every post-claim stage—including `claim_attachment`, `lockfile_plan_materialization` and `failure_recollection`—and every atomic operation has a deterministic persistent-claim/report/final-state contract.

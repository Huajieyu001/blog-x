...
|---|---|---|
| AUTH-01 | 唯一管理员可登录后台并保持安全会话，未登录访客无法访问受保护的管理功能。 | Database-backed opaque sessions, Argon2id password verification, protected page/API guard. |
| CONT-01 | 管理员可创建、编辑、预览、发布、下线和删除 Markdown 文章。 | Article state model, authenticated Route Handlers, shared Markdown renderer. |
...
| READ-01 | 访客可在首页查看已发布文章的标题、摘要、发布日期、分类、标签和分页信息。 | SSR public query, `publishedAt DESC`, deterministic cursor/page pagination; taxonomy wording needs resolution below. |
...
Build one Node.js-deployed Next.js App Router application in a pnpm workspace. Its Server Components own public SSR, while Route Handlers under `/api` are the same-origin backend-for-frontend boundary for login and content mutations. Next.js documents Route Handlers as custom handlers inside `app`, and its self-hosting guide supports putting the application behind a reverse proxy later. [CITED: https://nextjs.org/docs/app/getting-started/route-handlers] [CITED: https://nextjs.org/docs/app/guides/self-hosting]
...
**Primary recommendation:** Use a single `apps/blog` Next.js 16.3.0 application with internal `/api` Route Handlers, PostgreSQL + Drizzle migrations, database-backed opaque sessions, and server-rendered sanitized Markdown; use pnpm workspaces only, with no Turborepo, Redis, queue, or second application process in Phase 1. [CITED: https://nextjs.org/docs/app] [CITED: https://nextjs.org/docs/app/guides/self-hosting]

## Architectural Responsibility Map

| Capability | Primary Tier | Secondary Tier | Rationale |
|---|---|---|---|
| Public home and permalink rendering | Frontend Server (SSR) | Database / Storage | Server Components query only the public repository and render SEO-ready HTML without exposing privileged data. |
| Login and session validation | API / Backend | Database / Storage | Route Handlers create/revoke sessions; every protected request validates an opaque cookie against a server-side row. |
...
| Markdown rendering and sanitization | Frontend Server (SSR) | API / Backend | Render only on the server, with a single trusted transformation pipeline used by reading and preview. |
...
| Local orchestration and future proxy placement | CDN / Static | Frontend Server (SSR) | Local Compose supplies Postgres; a future reverse proxy can route same-origin traffic without changing browser URLs. |
...
|---|---:|---|---|
| `next`, `react`, `react-dom` | 16.3.0 (2026-08-03); 19.2.8 (2026-07-21) | One SSR UI and same-origin Route Handler API | App Router supplies Server Components and Route Handlers; no separate Node web/API processes are needed for the walking skeleton. [CITED: https://nextjs.org/docs/app] |
...
| `zod` | 4.4.3 (2026-05-04) | Request/form and configuration validation | One schema source for Route Handler input, seed configuration, and editor payloads. [ASSUMED] |
...
|---|---|---|
| One Next.js BFF | Separate Fastify/Nest API plus SPA | Stronger physical separation, but doubles local processes, routing, CORS/session surface, and deployment work before Phase 1 needs it. [ASSUMED] |
...
```bash
pnpm add next@16.3.0 react@19.2.8 react-dom@19.2.8 drizzle-orm@0.45.2 pg@8.22.0 @node-rs/argon2@2.0.2 zod@4.4.3 unified@11.0.5 remark-parse@11.0.0 remark-gfm@4.0.1 remark-rehype@11.1.2 rehype-sanitize@6.0.0 rehype-stringify@10.0.1 shiki@4.4.2
pnpm add -D drizzle-kit@0.31.10 @playwright/test@1.62.1
```
...
**Packages flagged as suspicious [SUS]:** all packages in the table because the seam could not retrieve age/download/repository metadata. Planner must add one `checkpoint:human-verify` immediately before the initial dependency installation; do not make one checkpoint per package.
...
```text
Browser
  | GET /, /posts/:slug ----------------------------> Next.js Server Components
  |                                                     | public-only query
  |                                                     v
  |                                              PostgreSQL articles
  |
  | POST /api/auth/login (credentials) -----------> Route Handler
  |                                                     | Argon2id verify
  |                                                     | create opaque session
  | <----------- Set-Cookie: session token ------------+
  |
  | POST/PATCH /api/admin/posts ------------------> Route Handler
  |          |                                          | validate cookie + Zod
  |          |                                          | transition state / slug
  |          |                                          v
  |          +-- authenticated preview ---------> shared server Markdown renderer
  |                                                     | GFM -> HAST -> sanitize -> highlight
  |                                                     v
  +<--------------- HTML preview / SSR article --------+

Local only: Docker Compose PostgreSQL <--- `DATABASE_URL` in ignored `.env.local`
Future: reverse proxy preserves one domain and routes the same application/API boundary.
```
...
│   └── api/                    # auth, admin post, preview, health handlers
├── src/
│   ├── auth/                   # password verification, session create/read/revoke
│   ├── content/                # article repository, state machine, markdown renderer
│   ├── db/                     # Drizzle client, schema, migration runner
│   └── lib/                    # config parsing, validation, date/slug helpers
...
**What:** Put every data read behind either `listPublicPosts`/`getPublicPostBySlug` or authenticated admin repository operations. Public predicates always require `status = published` and `deletedAt IS NULL`; admin preview is authorized before it loads any non-public row. [ASSUMED]
...
**What:** Store Markdown source unchanged, then use one asynchronous server pipeline for preview and public SSR: `remark-parse` → `remark-gfm` → `remark-rehype` with raw HTML disabled → Shiki code HAST transform → `rehype-sanitize` allowlist → `rehype-stringify`. Shiki provides server-side HTML output and advises reuse of a long-lived highlighter. [CITED: https://shiki.style/guide/install] [CITED: https://shiki.style/guide/best-performance]

**When to use:** Render preview through the same endpoint/module, with cancellation/debounce in the editor. Keep the Markdown renderer out of a Client Component and never enable raw HTML merely to support styling. [ASSUMED]

### Pattern 4: Opaque database session

**What:** Generate the session ID with Node's cryptographically secure random API, put only that opaque value in an HttpOnly cookie, and store a SHA-256 digest, administrator ID, expiry, and creation/revocation timestamps in PostgreSQL. Validate the exact generated value on every protected route; rotate/delete the old session on login and delete it on logout. OWASP says the application should accept only server-generated session IDs and that session identifiers must not enter browser storage. [CITED: https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html]
...
- **Standalone API plus browser-to-secondary-IP calls:** Violates the future same-domain boundary and adds CORS/cookie complexity before it provides value. [VERIFIED: docs/INFRASTRUCTURE.md:61-68]
...
### Pitfall 4: A costly slug change is silently accepted
...
### Pitfall 5: Local startup is claimed but cannot be reproduced
...
cookies().set("session", opaqueSessionId, {
  httpOnly: true,
  sameSite: "lax",
...
|---|---|---|---|
| Next.js Pages Router API Routes | App Router Route Handlers | Current Next.js documentation | Keep pages and BFF endpoints in `app/`; do not mix router paradigms for a greenfield app. [CITED: https://nextjs.org/docs/app/getting-started/route-handlers] |
...
| A3 | One Next.js BFF is the lowest-complexity path while still portable to the two-node target. | Architecture | A later Phase 4 deployment may need extraction of the API boundary. |
...
1. **READ-01 requires categories and tags, but the Phase 1 boundary defers category/tag management to Phase 2.**
   - What we know: `REQUIREMENTS.md` says **“READ-01: 访客可在首页查看已发布文章的标题、摘要、发布日期、分类、标签和分页信息。”**; `01-CONTEXT.md` says **“分类标签、图片上传、完整主题系统、SEO/RSS、备份恢复及生产部署分别属于后续阶段。”** and the traceability table maps **“TAXO-01 | Phase 2 | Pending”**. [VERIFIED: .planning/REQUIREMENTS.md:10,94] [VERIFIED: 01-CONTEXT.md:9]
   - What's unclear: Whether Phase 1 must add read-only placeholder fields/models (which would violate the stated boundary) or whether the requirements wording is to be satisfied in Phase 2.
   - Recommendation: Treat taxonomy as explicitly out of Phase 1, deliver title/excerpt/date/pagination now, and have the planner record READ-01's taxonomy subclause as deferred to the Phase 2 TAXO-01/reading work. Require a human decision if strict per-phase requirement coverage is enforced.

2. **Should a soft-deleted slug remain permanently reserved?**
...
|---|---|---:|---|---|
| Node.js | Next.js, migrations, tests | ✓ | v24.15.0 | — |
...
| V4 Access Control | yes | Guard every `/admin` page and mutation handler server-side; never authorize from client state. [ASSUMED] |
...
- [Next.js App Router](https://nextjs.org/docs/app) and [Route Handlers](https://nextjs.org/docs/app/getting-started/route-handlers) — SSR/App Router and same-origin HTTP handler conventions.

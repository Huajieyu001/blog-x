...
export async function generateMetadata({ params, searchParams }: {
  ...
  const [result, siteResult] = await Promise.all([
    getPublicTaxonomyPosts("tags", slug, parsed.success ? parsed.data.page : 1),
  ...
  return pageMetadata({ title: `${result.data.term.name} 标签`, description: `${result.data.term.name} 标签下的 ${result.data.posts.totalItems} 篇已发布文章。`, path: canonicalPath, index: canonical.index, site });
}

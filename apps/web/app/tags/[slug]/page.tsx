...
export default async function TagPage({
  ...
  if (!parsed.success) {
    return <main className={styles.page}><div className={styles.empty}><h1>页码无效</h1><p>请使用大于零的整数页码。</p><Link href="/" prefetch={false}>返回最新文章</Link></div></main>;
  }
...
export async function generateMetadata({ params, searchParams }: {
  ...
  const result = await getPublicTaxonomyPosts("tags", slug, parsed.success ? parsed.data.page : 1);
  if (result.kind === "not_found") notFound();
  if (result.kind === "upstream_error") throw new Error("public content unavailable");
  const path = `/tags/${encodeURIComponent(result.data.term.slug)}`;
  ...
  return pageMetadata({ title: `${result.data.term.name} 标签`, description: `${result.data.term.name} 标签下的 ${result.data.posts.totalItems} 篇已发布文章。`, path: canonicalPath, index: canonical.index });
}

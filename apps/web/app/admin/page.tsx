...
export default async function AdminPage({ searchParams }: AdminPageProps) {
  const initialFilter = parsePostFilter((await searchParams).status);
  ...
  return <main className={styles.workspace}>
    ...
    <section className={styles.workspaceSection} aria-labelledby="overview-title"><h2 id="overview-title">内容概况</h2>{content.kind === "upstream_error" ? <ContentFailure title="内容概况暂时不可用" /> : <div className={styles.overviewCards}>
      <a className={styles.overviewCard} href="/admin?status=published#articles" aria-current={initialFilter === "published" ? "true" : undefined}><h3>已发布</h3><strong>{published.length}</strong><p>当前对访客可见</p><span>查看文章 →</span></a>
      <a className={styles.overviewCard} href="/admin?status=draft#articles" aria-current={initialFilter === "draft" ? "true" : undefined}><h3>草稿</h3><strong>{drafts.length}</strong><p>{scheduled.length > 0 ? `其中 ${scheduled.length} 篇已预约` : "等待继续编辑"}</p><span>继续整理 →</span></a>
      <a className={styles.overviewCard} href="/admin?status=unpublished#articles" aria-current={initialFilter === "unpublished" ? "true" : undefined}><h3>已下线</h3><strong>{unpublished.length}</strong><p>保留内容，暂不公开</p><span>查看文章 →</span></a>
    </div>}</section>
    ...
    <section id="articles" className={styles.workspaceSection} aria-labelledby="posts-title"><h2 id="posts-title">文章管理{content.kind === "ok" ? ` · ${posts.length} 篇` : ""}</h2>{content.kind === "upstream_error" ? <ContentFailure title="暂时无法读取文章列表" /> : <AdminPostList posts={posts} initialFilter={initialFilter} />}</section>

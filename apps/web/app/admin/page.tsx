...
type PostFilter = "all" | "published" | "draft" | "unpublished" | "scheduled";
type PostSort = "recent" | "oldest" | "title";
const adminQueryMaximumLength = 160;

type AdminPageProps = {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
};

function parsePostFilter(value: string | string[] | undefined): PostFilter {
  return typeof value === "string" && ["published", "draft", "unpublished", "scheduled"].includes(value) ? value as PostFilter : "all";
}

function parsePostSort(value: string | string[] | undefined): PostSort {
  return typeof value === "string" && ["oldest", "title"].includes(value) ? value as PostSort : "recent";
}

function parseArticleQuery(value: string | string[] | undefined) {
  if (typeof value !== "string") return "";
  return value.normalize("NFC").trim().slice(0, adminQueryMaximumLength);
}

function overviewHref(status: Exclude<PostFilter, "all" | "scheduled">, sort: PostSort, query: string) {
  const parameters = new URLSearchParams();
  if (query) parameters.set("q", query);
  parameters.set("status", status);
  if (sort !== "recent") parameters.set("sort", sort);
  return `/admin?${parameters.toString()}#articles`;
}
...
export default async function AdminPage({ searchParams }: AdminPageProps) {
  ...
  const initialFilter = parsePostFilter(parameters.status);
  const initialSort = parsePostSort(parameters.sort);
  const initialQuery = parseArticleQuery(parameters.q);
  ...
  return <main className={styles.workspace}>
    ...
    <section className={styles.workspaceSection} aria-labelledby="overview-title"><h2 id="overview-title">内容概况</h2>{content.kind === "upstream_error" ? <ContentFailure title="内容概况暂时不可用" /> : <div className={styles.overviewCards}>
      <a className={styles.overviewCard} href={overviewHref("published", initialSort, initialQuery)} aria-current={initialFilter === "published" ? "true" : undefined}><h3>已发布</h3><strong>{published.length}</strong><p>当前对访客可见</p><span>查看文章 →</span></a>
      <a className={styles.overviewCard} href={overviewHref("draft", initialSort, initialQuery)} aria-current={initialFilter === "draft" ? "true" : undefined}><h3>草稿</h3><strong>{drafts.length}</strong><p>{scheduled.length > 0 ? `其中 ${scheduled.length} 篇已预约` : "等待继续编辑"}</p><span>继续整理 →</span></a>
      <a className={styles.overviewCard} href={overviewHref("unpublished", initialSort, initialQuery)} aria-current={initialFilter === "unpublished" ? "true" : undefined}><h3>已下线</h3><strong>{unpublished.length}</strong><p>保留内容，暂不公开</p><span>查看文章 →</span></a>
    </div>}</section>

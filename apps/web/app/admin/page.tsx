...
import AdminPostList, { AdminOverviewCards } from "./_components/AdminPostList";
...
export default async function AdminPage({ searchParams }: AdminPageProps) {
  ...
  return <main className={styles.workspace}>
    ...
    <section className={styles.workspaceSection} aria-labelledby="overview-title"><h2 id="overview-title">内容概况</h2>{content.kind === "upstream_error" ? <ContentFailure title="内容概况暂时不可用" /> : <AdminOverviewCards publishedCount={published.length} draftCount={drafts.length} unpublishedCount={unpublished.length} scheduledCount={scheduled.length} initialFilter={initialFilter} initialSort={initialSort} initialQuery={initialQuery} />}</section>

...
import ArticleRevisionHistory from "../../_components/ArticleRevisionHistory";
import { getAdminPostResult, getAdminRevisionListResult, getAdminTaxonomyResult } from "../../../lib/api";
...
export default async function EditDraftPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [post, categories, tags, revisions] = await Promise.all([
    ...
    getAdminRevisionListResult(id, cookie),
  ]);
  ...
  return (
    <>
      ...
      <ArticleRevisionHistory article={post.data} revisions={revisions.kind === "ok" ? revisions.data : []} unavailable={revisions.kind === "upstream_error"} />
      <noscript>

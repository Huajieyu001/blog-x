...
export default function TracerAdmin({ initiallyAuthenticated = false }: { initiallyAuthenticated?: boolean }) {
  ...
  async function login(form: FormData) {
    ...
    try {
      const response = await fetchWithDeadline("/api/auth/login", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(parsed.data), credentials: "same-origin" });
      ...
      setMessage(isFetchDeadlineExceeded(error) ? "登录请求超时，请刷新确认后再重试" : "登录服务暂时不可用");
    }
  }
  async function publish(form: FormData) {
    ...
    try {
      const response = await fetchWithDeadline("/api/articles/publish", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(parsed.data), credentials: "same-origin" });
      ...
      setMessage(isFetchDeadlineExceeded(error) ? "发布请求超时，服务器可能已完成发布；请刷新确认后再重试" : "发布服务暂时不可用");
    }
  }

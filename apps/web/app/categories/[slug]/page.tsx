...
export default async function CategoryPage({
  ...
  const result = await getPublicTaxonomyPosts("categories", slug, parsed.data.page);
  ...
  return <main className={styles.page}><section className={styles.feed}>
    <header className={styles.feedHeader}><p>分类</p><h1>{result.term.name}</h1><p>{result.posts.totalItems} 篇已发布文章</p></header>
    ...
    <Pagination page={result.posts.page} totalPages={result.posts.totalPages} basePath={`/categories/${encodeURIComponent(slug)}`} />
  </section></main>;
}

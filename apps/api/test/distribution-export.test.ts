...
test("the protected export reconstructs every retained source state without binary or infrastructure disclosure", async () => {
  ...
  try {
    ...
    const sourceMap = {
      articles: sourceArticles.map((item) => ({ ...item, publishedAt: item.publishedAt?.toISOString() ?? null, deletedAt: item.deletedAt?.toISOString() ?? null, createdAt: item.createdAt.toISOString(), updatedAt: item.updatedAt.toISOString(), tagIds: (sourceTagIds.get(item.id) ?? []).sort() })).sort((left, right) => left.id.localeCompare(right.id)),
    ...
    assert.doesNotMatch(JSON.stringify(manifest), /(?:sourceKey|derivativeKey|source_key|derivative_key|source\/|derivative\/|renderedHtml|base64|blob:|file:|password|session|administrator|postgres|127\.0\.0\.1|124\.222|47\.99)/i);

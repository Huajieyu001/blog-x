...
async function schemaVerify(pool: Pool) {
  const result = await pool.query("select tablename from pg_tables where schemaname = 'public' and tablename = any($1)", [["administrators", "sessions", "articles", "article_revisions", "article_slug_redirects", "article_daily_views", "categories", "tags", "article_tags", "site_pages", "site_settings", "media", "audit_events"]]);
  if (result.rowCount !== 13) throw new Error("article revisions schema is not active; run pnpm db:migrate first");
  const ledger = await pool.query("select migration_count from blog_x_schema_ledger where scope = 'phase1'");
  if (ledger.rowCount !== 1 || Number(ledger.rows[0]?.migration_count) !== 16) throw new Error("article revisions migration ledger is incomplete; run pnpm db:migrate first");
  const indices = await pool.query("select indexname from pg_indexes where schemaname = 'public' and indexname = any($1)", [["taxonomy_category_slug_unique", "taxonomy_tag_slug_unique", "article_tags_article_tag_unique", "articles_category_public_index", "site_pages_key_unique", "site_settings_singleton_unique", "media_source_key_unique", "media_derivative_key_unique", "media_catalog_retained_index", "articles_cover_media_index", "audit_events_newest_index", "articles_schedule_due_index", "article_daily_views_day_index", "article_slug_redirects_article_index", "article_revisions_article_source_version_unique", "article_revisions_newest_index"]]);
  if (indices.rowCount !== 16) throw new Error("required indexes are incomplete; run pnpm db:migrate first");
  const revisionConstraints = await pool.query("select conname from pg_constraint where conrelid = 'article_revisions'::regclass and conname = any($1)", [["article_revisions_pkey", "article_revisions_article_id_articles_id_fk"]]);
  if (revisionConstraints.rowCount !== 2) throw new Error("article revisions constraints are incomplete; run pnpm db:migrate first");
  ...
  const siteSettingsConstraints = await pool.query("select conname from pg_constraint where conrelid = 'site_settings'::regclass and conname = any($1)", [["site_settings_name_check", "site_settings_registration_number_check"]]);
  if (siteSettingsConstraints.rowCount !== 2) throw new Error("site settings constraints are incomplete; run pnpm db:migrate first");
  ...
  if (![
    ["audit_events_event_check", "article.scheduled", "article.rescheduled", "article.schedule_cancelled", "article.scheduled_published", "auth.password.changed", "media.deleted", "site_settings.updated"],
    ["audit_events_target_check", "auth.password.changed", "administrator", "actor_administrator_id", "site_settings"],

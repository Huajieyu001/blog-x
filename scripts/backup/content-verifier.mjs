...
export const backupMemberPattern = /^(?:database\.dump|portable-export-v1\.json|config\/inventory\.json|media\/(?:source|derivative)\/[0-9a-f-]{36}\.(?:bin|jpg|png|webp))$/i;
...
export const backupManifestSchema = {
  parse(value) {
    ...
    for (const member of value.members) {
      ...
      if (!backupMemberPattern.test(member.path) || member.path.includes("..") || seen.has(member.path) || member.path <= previous
...
export async function listBackupFiles(root, directory = root) {
  const output = [];
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (info.isDirectory()) output.push(...await listBackupFiles(root, path));
...
async function assertNotLink(value, label) {
...
function parseInventory(value) {
  ...
  strictObject(value.migration, ["count", "fingerprint"], "migration inventory");
  strictObject(value.images, ["api", "postgres", "web"], "image inventory");
  if (!Number.isSafeInteger(value.migration.count) || value.migration.count <= 0 || !hashPattern.test(value.migration.fingerprint)
    || Object.values(value.images).some((digest) => typeof digest !== "string" || !/^sha256:[a-f0-9]{64}$/.test(digest))) throw new Error("config inventory is invalid");
  ...
  for (const item of value.media) {
    ...
    if (!/^[0-9a-f-]{36}$/i.test(item.id) || !hashPattern.test(item.sourceSha256) || !hashPattern.test(item.derivativeSha256)
      || !backupMemberPattern.test(item.sourcePath) || !backupMemberPattern.test(item.derivativePath)) throw new Error("media inventory is invalid");
  }
  return value;
}

export async function verifyCompleteBackupSetContents(root, rootValidator) {
  if (typeof rootValidator !== "function") throw new Error("root validator is required");
  const backupRoot = await rootValidator(root);
  if (typeof backupRoot !== "string" || !backupRoot) throw new Error("root validator returned an invalid backup root");
  ...
  const actual = await listBackupFiles(backupRoot);

...
test("source contracts require neutral stores, offline frozen installs and sanitized refresh workspaces", async () => {
  ...
  assert.ok(webDockerfile.indexOf("ARG REFRESH_REVISION") > webDockerfile.indexOf("RUN corepack pnpm --filter @blog-x/web build"));
  ...
  assert.ok(apiDockerfile.indexOf("ARG REFRESH_REVISION") > apiDockerfile.indexOf("COPY packages/contracts packages/contracts"));

...
test("Phase 15 media selector is sealed to generated media API and browser authority", async () => {
  ...
  assert.match(mediaSpec, /getByRole\("listitem"\)\.filter\(\{ hasText: purposefulId \}\)/);
  assert.match(mediaSpec, /getByRole\("listitem"\)\.filter\(\{ hasText: unusedId \}\)/);
...
test("current migration authority is a complete sixteen-entry Drizzle history", async () => {
  ...
  for (const token of [
    ...
    if (!mediaMigration.includes(token)) findings.push(`managed-media migration is missing ${token}`);
  ...
  for (const token of ["site_settings", "site_settings_singleton_unique", "site_settings_registration_number_check", "site_settings.updated"]) {
    if (!settingsMigration.includes(token)) findings.push(`site-settings migration is missing ${token}`);
    if (!schema.includes(token)) findings.push(`Drizzle schema is missing ${token}`);
  }
...
test("canonical generated production inventory declares current migration count 11", async () => {
  const runner = await readFile(join(process.cwd(), "scripts/local-verify.mjs"), "utf8");
  ...
  assert.match(pipeline, /migration:\s*\{\s*count:\s*11,/);
  assert.doesNotMatch(pipeline, /migration:\s*\{\s*count:\s*10,/);
});

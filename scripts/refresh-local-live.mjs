...
export function createRefreshAttemptStore({ fs = nativeFs, identity = { uid: process.getuid?.() }, randomHex = () => randomBytes(12).toString("hex"), ...unexpected } = {}) {
  ...
  async function publishFile(finalPath, bytes, suffix) {
    const tempPath = `${CLAIM_ROOT}/.${basename(finalPath)}.${suffix}.tmp`;
    let handle; let tempExists = false;
    try {
      ...
      const finalDirectory = await fs.open(CLAIM_ROOT, "r");
      try { await finalDirectory.sync(); } finally { await finalDirectory.close(); }
    } finally {
      if (handle) await handle.close();
      if (tempExists) await fs.unlink(tempPath);
    }
  }
  return Object.freeze({
    ...
    async claimRefreshAttempt(revision) {
      ...
      await publishFile(finalPath, bytes, suffix);
    ...
    async assertFailureReportPresent(revision) {
      const path = failurePathFor(revision);
      if (!(await assertRoot())) fail("refresh failure report is absent");
      await assertClaimFile(path);
      const bytes = await fs.readFile(path, "utf8");
    ...
    async writeFailureReport(report) {
      ...
      await publishFile(path, bytes, suffix);
...
export function createRefreshFactSources({ run, fetch, root = process.cwd(), fs = nativeFs, seedFacts = () => ({ api: { reference: "unresolved", inspectedId: "sha256:" + "0".repeat(64) }, web: { reference: "unresolved", inspectedId: "sha256:" + "0".repeat(64) } }), targetFacts = () => ({ api: null, web: null }) }) {
  return Object.freeze({
    ...
    async seeds() { return structuredClone(seedFacts()); },
    async targets() { return structuredClone(targetFacts()); },
...
async function publishEvidence(fs, finalPath, bytes, randomHex) {
  ...
  try {
    ...
    if (handle) {
      try { await handle.close(); handle = undefined; } catch (closeError) { throw closeError; }
    }
    if (linked) {
      try { await fs.unlink(finalPath); linked = false; await syncDirectory(); }
      catch (cleanupError) { throw new Error(`UNRECOVERABLE_EVIDENCE_INVARIANT:${safeCode(cleanupError)}`, { cause: error }); }
    }
    if (tempExists) { await fs.unlink(temp); tempExists = false; }
    throw error;
  }
}

export function createLiveRefreshAdapter({ runArgv, claimStore = createRefreshAttemptStore(), fetch = globalThis.fetch, root = process.cwd(), evidenceFs = nativeFs, collectFacts, targetProbe, randomEvidenceHex = () => randomBytes(8).toString("hex"), ambientEnv = {}, enforceLocalAuthority = false } = {}) {
  ...
  const state = { facts: {}, claim: undefined, targets: {}, seeds: { api: null, web: null }, oldImages: {}, cutover: false, migrationOneoff: undefined, phase: "constructed" };
  let sources; let collect;
  const initializeCollectors = () => {
    if (sources) return;
    sources = createRefreshFactSources({ run, fetch, root, fs: evidenceFs, seedFacts: () => state.seeds, targetFacts: () => Object.fromEntries(["api", "web"].map((app) => [app, state.targets[app] ? { id: state.targets[app].Id, labelsSha256: factsSha256(selectedLabels(state.targets[app].Config?.Labels)), filesystemSha256: state.targets[app].probe?.filesystemSha256 ?? null, storeSha256: state.targets[app].probe?.storeSha256 ?? null } : null])) });
    collect = collectFacts ?? (() => collectRefreshFacts({ sources }));
  };
...
export async function verifyLiveRefreshEvidence(path, { claimStore = createRefreshAttemptStore(), fs = nativeFs, collectFacts, probeTargets, runArgv, fetch = globalThis.fetch, root = process.cwd() } = {}) {
  ...
  const reconstructedTargetFacts = {};
  const reconstruct = collectFacts ?? (run ? () => collectRefreshFacts({ sources: createRefreshFactSources({ run, fetch, root, fs, seedFacts: () => evidence.seeds, targetFacts: () => reconstructedTargetFacts }) }) : undefined);
  const probe = probeTargets ?? (run ? async (targets) => {
    for (const app of ["api", "web"]) {
      ...
      reconstructedTargetFacts[app] = { id: image.Id, labelsSha256: factsSha256(selectedLabels(image.Config?.Labels)), filesystemSha256, storeSha256: digest(store) };
    }
  } : undefined);
  ...
  if (run) current.git = verifiedGit;
...
function nativeProductionRun(command, args, options = {}) {
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    child.once("error", reject); child.once("close", (code) => code === 0 ? resolvePromise({ stdout, stderr }) : reject(new Error(`${command} failed with code ${code}`)));
...
export function createProductionLiveRefreshAdapter(...args) {
  ...
  return createLiveRefreshAdapter({ runArgv: nativeProductionRun, claimStore: createProductionRefreshAttemptStore(), fetch: globalThis.fetch, root: productionRoot, evidenceFs: nativeFs, ambientEnv: process.env, enforceLocalAuthority: true });
}
...
export function verifyProductionLiveRefreshEvidence(...args) {
  ...
  return verifyLiveRefreshEvidence(resolve(productionRoot, EVIDENCE_PATH), { claimStore: createProductionRefreshAttemptStore(), fs: nativeFs, runArgv: nativeProductionRun, fetch: globalThis.fetch, root: productionRoot });
}
